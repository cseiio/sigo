...
class BuzonInterno extends CI_Controller {
	...
	public function index()
	{
		if ($this->session->userdata('area_trabajo')) {
			...
			$data['entradas'] = $this -> Modelo_direccion -> getBuzonDeOficiosEntrantes('1');
			...
			$data['nums_circular']  = $this -> Modelo_direccion -> getNumsCircular('DIRECCIÓN GENERAL');

			$data['nums_memorandums_planteles']  = $this -> Modelo_direccion -> getNumsMemorandumsPlanteles('DIRECCIÓN GENERAL');
			$data['nums_oficio_planteles']  = $this -> Modelo_direccion -> getNumsOficiosUsadosPlanteles('DIRECCIÓN GENERAL');
			$data['nums_circular_planteles']  = $this -> Modelo_direccion -> getNumsCircularPlanteles('DIRECCIÓN GENERAL');
			...
			$consulta = $this -> Modelo_direccion -> getBuzonDeOficiosEntrantes('1');
			...
			$this->load->view('recepcion/dirgral/internos/index');
		...
		public function asignarOficio()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data['entradas'] = $this -> Modelo_direccion -> getAllEntradas('1');
			$data['deptos'] = $this -> Modelo_direccion -> getDeptos('1');
			$this->load->view('plantilla/header', $data);
					...
					if ($num_deptos_en_bd > $num_deptos_en_formulario) {
						...
						foreach ($deptosconsultados as $deptosbd) {
							...
							if ($encontrado == false){
								...
								$eliminarDeptos = $this->Modelo_direccion->eliminarAsignacionDeptoInterna($deptosbd);
							}
						}
						if ($eliminarDeptos) {
