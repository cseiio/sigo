...
class ReportesEmitidos extends CI_Controller {
	...
	public function index()
	{
		if ($this->session->userdata('area_trabajo')) {
			...
			$data['deptos'] = $this -> Modelo_direccion -> getDeptos('1');
      $data['dependencias'] = $this -> Modelo_direccion-> getAllDependenciasCSEIIO();
			$this->load->view('plantilla/header', $data);
			$this->load->view('recepcion/dirgral/internos/reportesemitidos');
...
function getDiasHabiles($fechainicio, $fechafin, $diasferiados = array()) {
  ...
  for ($midia = $fechainicio; $midia <= $fechafin; $midia += $diainc) {
        ...
        //$hoy = date("F j, Y, g:i a");
             ...
             $id_direccion = $this->session->userdata('1');
...
// Se agregan los titulos del reporte
 ...
 public function reporteEmitidosDireccionesInt()
 {
   ...
   $titulosColumnas = array('NÚMERO DE OFICIO', 'FECHA DE EMISIÓN', 'HORA DE EMISIÓN', 'FECHA DE ACUSE DE RECEPCIÓN','HORA DE ACUSE DE RECEPCIÓN','ASUNTO', 'EMISOR', 'CARGO','FECHA DE TERMINO','ESTATUS','DIRECCIÓN DE DESTINO','OBSERVACIONES','N° DE OFICIO DE RESPUESTA','FECHA DE RESPUESTA','HORA DE RESPUESTA','PERSONA QUE CONTESTO EL OFICIO','CÓDIGO ARCHIVISTICO','FECHA DE SUBIDA','HORA DE SUBIDA','VALOR','VIGENCIA','CLASIFICACIÓN','TIPO DE DOCUMENTO ARCHIVISTICO');

$objPHPExcel->getActiveSheet(0)->mergeCells('A3:W3');
    $objPHPExcel->getActiveSheet(0)->mergeCells('A4:W4');
        // Se combinan las celdas A1 hasta F1, para colocar ahí el titulo del reporte
   $objPHPExcel->setActiveSheetIndex(0)
   ->mergeCells('A1:W1');
// Se agregan los titulos del reporte
  ...
  $style_contenido = array('alignment'=> array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER), 'font' => array('size' => 12,'bold' => false,'color' => array('rgb' => '000000')));
   ...
   $objPHPExcel->getActiveSheet()->getStyle('A6:W6')->applyFromArray($styleA6R6);

    $objPHPExcel->getActiveSheet()->getStyle('A6:W6')->getFill()
    ->setFillType(PHPExcel_Style_Fill::FILL_SOLID)
    ->getStartColor()->setRGB('579A8D');
    ...
    foreach($oficios as $fila)
    {
      $objPHPExcel->getActiveSheet()->getStyle('A'.$i.':'.'W'.$i)->applyFromArray($style_contenido);
      ...
      if ($fila->status == 'Contestado' OR $fila->status == 'Fuera de Tiempo') {
        ...
        foreach ($infor_contestados  as $key) {
          ...
          ->setCellValue('R'.$i, $key->hora_subida)
          ->setCellValue('S'.$i, $key->fecha_subida)
          ->setCellValue('T'.$i, $key->valor_doc)
          ->setCellValue('U'.$i, $key->vigencia_doc)
          ->setCellValue('V'.$i, $key->clasificacion_info)
          ->setCellValue('W'.$i, $key->tipo_doc_archivistico);
        }
      }
      else
      {
        ...
        ->setCellValue('R'.$i, $fila->hora_subida)
        ->setCellValue('S'.$i, $fila->fecha_subida)
         ->setCellValue('T'.$i, $fila->valor_doc)
          ->setCellValue('U'.$i, $fila->vigencia_doc)
          ->setCellValue('V'.$i, $fila->clasificacion_info)
          ->setCellValue('W'.$i, $fila->tipo_doc_archivistico);
      }
    ...
    $oficios = $this->Modelo_reportes->getAllOficiosEmitidosDirIntByDependencias($nombre, $inicio, $final, $dependencia);
  ...
  public function reporteContestadosDireccionesInt() {
    ...
    $inicio = $this->input->post('date_inicio');
    $final = $this->input->post('date_final');
    $id_direccion = 'DIRECCIÓN GENERAL';
